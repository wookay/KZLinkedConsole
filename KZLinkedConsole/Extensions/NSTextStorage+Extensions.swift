...
extension NSTextStorage {
    ...
    var kz_isUsedInXcodeConsole: Bool {
        ...
        set {
            objc_setAssociatedObject(self, &AssociatedKeys.isConsoleKey, NSNumber(value: newValue as Bool), .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
...
final class KZLinkInjector {
    ...
    /**
     ...
     is a complete guess.
     */
    fileprivate let syncThreshold = 2000

    fileprivate let pendingLinksBatchTime = DispatchTime.now() + Double(300000000) / Double(NSEC_PER_SEC)
    ...
    */
    fileprivate unowned let textStorage : NSTextStorage
    ...
    */
    fileprivate var pendingLinks : [LinkDetails] = []
    ...
    /**
     ...
     May only be accessed under pendingLinksLock.
     */
    fileprivate var pendingLinksCounter = 1

    fileprivate let pendingLinksLock = NSLock()
     ...
     */
    fileprivate func appendPendingLinks(_ newLinks : [LinkDetails]) -> Int {
     ...
     */
    fileprivate func getPendingLinksMatchingCounter(_ myPendingLinksCounter : Int) -> [LinkDetails]? {
    ...
    fileprivate func injectLinksIntoTextSync(_ range : NSRange) {
    ...
    fileprivate func injectLinksIntoTextAsync(_ range : NSRange) {
    ...
    fileprivate func injectLinksIntoTextBackground(_ myText : String, range : NSRange) {
    ...
    fileprivate func handlePendingLinksAfterDelay(_ myPendingLinksCounter : Int) {
    ...
    fileprivate func handlePendingLinks(_ myPendingLinksCounter : Int) {
    ...
    fileprivate func addLinksToTextStorage(_ links : [LinkDetails]) {
    ...
    fileprivate static func findLinksInText(_ string : String, range : NSRange) -> [LinkDetails]? {
    ...
    fileprivate static var pattern: NSRegularExpression {
    ...
    fileprivate static func withLock<T>(_ lock: NSLock, block: () -> T) -> T {
        lock.lock()
        let result = block()
        lock.unlock()
        return result
